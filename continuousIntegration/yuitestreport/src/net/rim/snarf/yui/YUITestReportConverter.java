...
public class YUITestReportConverter {
	...
	void createTestReport(Element testsuiteElement) throws Exception{
		...
		NodeList failureNodeList=(NodeList)xPath.evaluate("/testsuite/testcase/failure", testReportDoc, XPathConstants.NODESET);
		System.out.println(failureNodeList.getLength());
		for (int i=0; i<failureNodeList.getLength(); i++) {
            Node node=failureNodeList.item( i );
            if (node instanceof Element) {
            	((Element)node).setAttribute("type","error");
            }
        }
	...
	public static void main(String[] args) {
		...
		if ((args != null) && (args.length > 0)) {
			...
			yuitestreport="C:/mvn_local/1.0.0.8-soa1.0-yuiTests.xml";
		}
