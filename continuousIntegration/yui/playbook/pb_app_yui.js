(function() {
	...
	function generateTestCaseCallback(Y) {
		...
		var Assert = Y.Assert;
			...
			"blackberry.app.event.onSwipeStart() should exist and be type of function": function() {
				Assert.isNotUndefined(blackberry.app.event.onSwipeStart);
				Assert.isFunction(blackberry.app.event.onSwipeStart);  
			},
			
			"blackberry.app.event.onSwipeDown() should exist and be type of function": function() {
				Assert.isNotUndefined(blackberry.app.event.onSwipeDown);
				Assert.isFunction(blackberry.app.event.onSwipeDown);  
			},
			...
			//onForeground (set)
			"MANUAL blackberry.app.event onForeground event should trigger when application is set to foreground": function() {
				//alert('ppp');
				framework.test = this; //so pass() and fail() can access this test
			...
			"MANUAL blackberry.app.event.onSwipeStart() should callback when the user initiates a swipe from the bezel" : function(){
				framework.test = this; //so pass() and fail() can access this test
				framework.setInstructions("Swipe down from the top of the bezel towards the screen, this should fire swipeStart");
				
				blackberry.app.event.onSwipeStart(function(){
					framework.test.resume(); //test passes, callback occured
				});
				
				framework.test.wait(function(){
					Assert.fail("Swipe start did not occur within 5min limitation");
				}, 5*60*1000);
			},
			
			"MANUAL blackberry.app.event.onSwipeDown() should callback when the user swipes down from the bezel" : function(){
				framework.test = this; //so pass() and fail() can access this test
				framework.setInstructions("Swipe down from the top of the bezel towards the screen, this should fire swipeDown");
				...
				framework.test.wait(function(){
					Assert.fail("Swipe down did not occur within 5min limitation");
				}, 5*60*1000);
			}
